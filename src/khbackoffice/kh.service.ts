...
export class KhService {
  ...
  async findStatement(dto: { noId: string; operator: string }): Promise<any | null> {
    const query = `
      SELECT *
      FROM dw_kh_backoffice_statements
      WHERE no_id = $1
        AND operator = $2
      LIMIT 1
    `;

    const params = [dto.noId, dto.operator];

    const result = await this.dwConnection.query(query, params);
    return result[0] || null; // return single row or null
  }

  async insertKhRobotTransaction(dto: {
    relatedUser?: string;
    amount: number;
    currency?: string;
    ip?: string;
    ...
    const query = `
      INSERT INTO dw_kh_backoffice_statements (
        related_user,
        amount,
        currency,
        ip,
    ...
    const params = [
      ...
      dto.bankAccountTranslate,
      dto.bankAccountType,
      dto.transferId,
      dto.transferType,
      dto.userBankName,
      dto.userBankAccountName,
      dto.userBankNo,
